        ...
        try {
            ...
            for (int i = 0; i < indexCount; i++) {
                ...
                if (attr == R.styleable.EasySwipeMenuLayout_leftMenuView) {
                    ...
                    mFraction = typedArray.getFloat(R.styleable.EasySwipeMenuLayout_fraction, 0.2f);
                }
            }
...
//                    getParent().requestDisallowInterceptTouchEvent(true);
    ...
    public State isShouldOpen(int scrollX) {
        if (getScrollX() < 0 && mLeftView != null) {
            ...
            //⬅️滑动
            if (Math.abs(mRightView.getWidth() * 0.1f) < Math.abs(getScrollX())) {
