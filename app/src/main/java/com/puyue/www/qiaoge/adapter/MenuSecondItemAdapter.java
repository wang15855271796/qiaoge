 ...
 */
public class MenuSecondItemAdapter extends BaseAdapter {

    //上下文
    private Context mContext;
    //菜单列表
    private List<CityChangeModel.DataBean.CityNamesBean> mListData;
    ...
    public MenuSecondItemAdapter(Context context, List<CityChangeModel.DataBean.CityNamesBean> listData, int sId, int nId) {
        mContext = context;
        mListData = listData;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (mListData != null) {
            if (position < mListData.size()) {
                mString = mListData.get(position).getCityName();
